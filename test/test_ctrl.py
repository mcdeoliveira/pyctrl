...
class BaseTest(unittest.TestCase):
    ...
    def run_test_basic(self, controller):
        ...
        self.assertTrue(
            controller.get_sink('_logger_') == {'current': 0, 'auto_reset': False, 'page': 0, 'enabled': True,
                                                'labels': ['_test_'], 'index': None})
        ...
        controller.add_sink('_printer_', block.Printer(), ['_test_', '_output_'])
    ...
    def run_test_timer(self, controller):
        ...
        self.assertTrue(controller.get_signal('timer') == 0)

        self.assertTrue(controller.get_timer('timer') == {'enabled': True, 'demux': False, 'mux': False, 'value': 1})

        self.assertTrue(controller.get_timer('timer', 'enabled', 'demux') == {'enabled': True, 'demux': False})

        self.assertTrue(controller.get_timer('timer', 'enabled') == True)

        with controller:
            time.sleep(2)

        self.assertTrue(controller.get_signal('timer') == 1)
    ...
    def run_test_set(self, controller):
        ...
        self.assertTrue(controller.get_source('const', 'value') == 1)

        with controller:
            controller.set_signal('s2', 1)
            time.sleep(.5)

        self.assertTrue(controller.get_signal('s2') == 1)
        self.assertTrue(controller.get_source('const', 'value') == 0.6)
